...
@Slf4j
public class ModifyGenericData extends ModifyItem {

	private Logger logger = LoggerFactory.getLogger(ModifyGenericData.class);

	//Any Action class inheriting from ModifyGenericData
	...
	public void preSave(XFTItem item,RunData data, Context context) throws Exception{
		try {
			...
			XnatGenericdata imageAssessorData = (XnatGenericdata) BaseElement.GetGeneratedItem(item);
	...
	public interface PreSaveAction {
		void execute(UserI user, XnatGenericdata src, Map<String, String> params, PersistentWorkflowI wrk) throws Exception;
	}

	private void dynamicPreSave(UserI user, XnatGenericdata src, Map<String,String> params,PersistentWorkflowI wrk) throws Exception{
		List<Class<?>> classes = Reflection.getClassesForPackage("org.nrg.xnat.actions.genericDataEdit.preSave");

		if(classes!=null && classes.size()>0){
			for(Class<?> clazz: classes){
				if(ModifyGenericData.PreSaveAction.class.isAssignableFrom(clazz)){
					ModifyGenericData.PreSaveAction action=(ModifyGenericData.PreSaveAction)clazz.newInstance();
