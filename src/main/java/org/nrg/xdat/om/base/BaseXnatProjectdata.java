...
public class BaseXnatProjectdata extends AutoXnatProjectdata implements ArchivableItem {
    ...
    public XFTTable getExperimentsByProject() throws DBPoolException, SQLException {
        final XFTTable experimentTable = XFTTable.Execute("SELECT shr.label, exp.project AS origProject, exp.label AS originalLabel FROM xnat_experimentdata_share shr LEFT JOIN xnat_experimentdata AS exp ON shr.sharing_share_xnat_experimentda_id= exp.id WHERE shr.project='" + getId() + "' UNION SELECT label, project AS origProject, label as originalLabel FROM xnat_experimentdata WHERE project='" + getId() + "';", getDBName(), null);
