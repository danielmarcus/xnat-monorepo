/*
 * xnat-data-models: org.nrg.xdat.om.base.BaseXnatImagescandata
...
public class BaseXnatImagescandata extends AutoXnatImagescandata {
    ...
    public void deleteFilesFromFileSystem(String rootPath,UserI u, EventMetaI ci) throws Exception{
        ...
        if (!files.isEmpty()) {
			...
			for (XnatAbstractresourceI resource : files) {
				((XnatAbstractresource) resource).deleteWithBackup(rootPath, project, u, ci, timestamp);
			}
		}
    }
    ...
    public Snapshot getSnapshot(){
		...
		if(this.getFile().size()>0){
		    ...
		    String project = getProject();
		    String archivePath = ses.getProjectData().getRootArchivePath();
		    String path=null;
	...
	public void validate(String expectedPath) throws Exception{
		...
		if(StringUtils.isBlank(this.getId())){
			throw new IllegalArgumentException("No ID specified for the path " + expectedPath + " on scan " + getId() + " (" + StringUtils.defaultIfBlank(getType(), getSeriesDescription()) + ") for session " + getImageSessionId() + " in project " + getProject());
		}	
		
		if(!XftStringUtils.isValidId(getId())){
			throw new IllegalArgumentException("Identifiers cannot use special characters: " + getId());
		}
		...
		for(final XnatAbstractresourceI res: this.getFile()){
			...
			FileUtils.ValidateUriAgainstRoot(uri, expectedPath,"URI references data outside of the project: " + uri + ", expected path is: " + expectedPath);
		}
	}
	...
	public static List<XnatImagescandata> getScansByIdORType(final String scanID,
		...
		final boolean isSingleNonWildcard = !StringUtils.equalsAny(scanID, "*", "ALL") && !StringUtils.contains(scanID, ",");
	            ...
	            if(resource instanceof XnatResourcecatalogI resourcecatalogI){
	               // Get the rootPath and FileStats
					try {
						...
						logger.error("Unable to read catalog for the given resource", e);
					}
	            }else{
	...
	public static void SaveSharedProject(XnatImagescandataShare pp, XnatImagescandata scan, UserI user,
										 final EventDetails event) throws Exception {
		String sessionId = scan.getImageSessionId();
		XnatExperimentdata session = XnatExperimentdata.getXnatExperimentdatasById(sessionId, user,
				false);
		if (session == null) {
			throw new NotFoundException("No experiment with ID " + sessionId);
		}
		PersistentWorkflowI wrk= WorkflowUtils.buildOpenWorkflow(user, session.getItem(), event);
