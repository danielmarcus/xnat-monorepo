...
public class IniImporter {
            ...
            try {
                ...
                for (final Resource resource : BasicXnatResourceLocator.getResources(iniFileSpec)) {
                    try {
                        ...
                        log.info("Resource {} not found. This is probably OK, sometimes Spring makes stuff up, so we'll just skip it.", resource.getFilename());
                    }
                }
            } catch (IOException | ConfigurationException e) {
                log.error("An error occurred trying to read the ini file {}", iniFileSpec, e);
            }
        }
    }
    ...
    private static void loadIni(final IniStore store, final Reader reader, final String name, final String[] requested) throws ConfigurationException, IOException {
        if (name.endsWith(".ini")) {
            ...
            log.info("Found specified init file {}, but it doesn't end in '.ini' or '.properties', so I'm not sure what to do with it.", name);
        }
    }
    ...
    private static void loadIni(final IniStore store, final Reader reader, final String... requested) throws ConfigurationException, IOException {
        ...
        final Set<String> sections = requested.length == 0 ? ini.getSections() : Arrays.stream(requested).collect(Collectors.toSet());
    ...
    private static void loadProperties(final IniStore store, final Reader reader, final String... requested) throws IOException {
        final Properties properties = new Properties();
        properties.load(reader);

        final Set<String> sections = Arrays.stream(requested).collect(Collectors.toSet());
    ...
    private static class MapIniStore implements IniStore {
        ...
        public void storeIniValue(final String section, final String key, final String value) {
            if (!_iniMap.containsKey(section)) {
                final Map<String, String> sectionMap = new HashMap<>();
        ...
        private final Map<String, Map<String, String>> _iniMap = new HashMap<>();
    }
