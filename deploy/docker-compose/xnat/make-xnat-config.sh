#!/bin/sh
# Generate xnat-conf.properties from environment variables.
# Called at build time to create a default config.
# At runtime, a bind-mounted config file will override this.

set -e

# Write config only if it does not already exist
...
# Optional: admin email prefs
if [ -n "$XNAT_EMAIL" ]; then
...
# Stage config files so they can be restored if a volume mount clobbers them
